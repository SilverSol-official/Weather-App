...
export const fetchWeather = createAsyncThunk(
    ...
    async function(city='Kharkiv',{rejectWithValue}){
...
export const fetchForecast = createAsyncThunk(
    ...
    async function(city='Kharkiv',{rejectWithValue}){
        // const url = `https://api.openweathermap.org/data/2.5/forecast?q=${city}&lang=en&units=metric&APPID=b286e981ec3e548cd8a4d987628524d4`;
        const url = `http://api.weatherapi.com/v1/forecast.json?key=e774f568556c4160968133110231902&q=${city}&days=10&aqi=no&alerts=no`
        try{
            const responce = await fetch(url);
            if (!responce.ok){
                throw new Error('error');
